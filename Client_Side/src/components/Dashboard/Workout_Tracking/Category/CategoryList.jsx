...
function CategoryList() {
  ...
  const SortableHeader = ({ column, children }) => {
    ...
    return (
      <th
        className="px-4 py-2 cursor-pointer whitespace-nowrap bg-bg_black text-textgray border  border-black"
  ...
  return (
    <div className="flex justify-center py-8 bg-black h-screen font-oswald">
      <ToastContainer />
      <div className="max-w-2xl w-full mx-auto p-4 bg-bg_black text-textgray rounded-lg shadow-md">
        <h1 className="text-3xl font-semibold mb-6 text-textgray text-start">Categories</h1>
        ...
        <div className="flex mb-4 items-center justify-between">
          <div className="flex items-center">
            <input
              ...
              className="px-4 py-2 mr-2 border border-bg_black rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-orangecolor focus:border-transparent"
            />
          </div>
          <Link to="/db/createCategory" className="bg-orangecolor text-white px-4 py-2 rounded flex items-center hover:bg-bg_black transition-all duration-500 focus:outline-none">
        ...
        {loading ? (
          ...
          <>
            {categories.length === 0 ? (
              ...
              <div className="overflow-x-auto mt-4 border border-black">
                <table className="w-full table-auto text-sm md:text-base ">
                  <thead className="text-center border border-black  ">
                    <tr className="border border-black">
                      <SortableHeader column="name">Category Name</SortableHeader>
                      <th className="px-4 py-2 bg-bg_black border border-black">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y border-black">
                    {currentCategories.map((category) => (
                      <tr key={category._id} className=" d  divide-black text-center border-black">
                        <td className="px-4 py-2 border-2 border-black ">{category.name}</td>
