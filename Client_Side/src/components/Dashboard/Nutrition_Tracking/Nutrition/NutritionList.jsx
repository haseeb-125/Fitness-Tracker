...
function NutritionList() {
    ...
    <tbody>
        {currentNutritions.map(item => (
            <tr key={item._id} className="bg-white hover:bg-gray-100 border-b border-gray-300">
                <td className="px-4 py-2 border  bg-bg_black border-black">{mealTypes.find(mealType => mealType._id === item.mealType)?.name}</td>
                <td className="px-4 py-2 border bg-bg_black border-black">{item.name}</td>
                <td className="px-4 py-2 border bg-bg_black border-black">{item.quantity}</td>
                <td className="px-4 py-2 border bg-bg_black border-black">{item.calories}</td>
                <td className="px-4 py-2 border bg-bg_black border-black">{item.carbohydrates}</td>
                <td className="px-4 py-2 border bg-bg_black border-black">{item.proteins}</td>
                <td className="px-4 py-2 border bg-bg_black border-black">{item.fats}</td>
                <td className="px-4 py-2 border bg-bg_black border-black">{formatDateTime(item.createdAt)}</td>
                <td className="px-4 py-2 border bg-bg_black border-black">{formatDateTime(item.updatedAt)}</td>
                <td className="px-4 py-2  bg-bg_black">
                    <button
  ...
  const SortableHeader = ({ column, children }) => {
    ...
    return (
      <th
        className="px-4 py-2 cursor-pointer whitespace-nowrap border-b border-black"
  ...
  return (
    <div className="flex items-start font-oswald text-textgray justify-center h-screen bg-black">
      <ToastContainer />
      <div className="max-w-4xl w-full mx-auto mt-8 p-4 bg-bg_black rounded-lg shadow-md">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-1 mb-4">
          <div className="flex flex-col md:flex-row items-center mb-2 md:mb-0">
            <input
              ...
              className="border border-black rounded px-3 py-2 focus:outline-none focus:border-orangecolor mr-2 mb-2 md:mb-0"
            />
            <select
              value={filterMealType}
              onChange={handleFilter}
              className="border border-black rounded px-3 py-2 focus:outline-none focus:border-orangecolor mb-2 md:mb-0"
            >
